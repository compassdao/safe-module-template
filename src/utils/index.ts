...
export const getTemplates = (
  ...
  if (masterTemplatesController?.open) {
    if (chainId) {
      const currentNetworkTemplateController =
        masterTemplatesController?.templates[chainId]
      if (
        currentNetworkTemplateController?.open &&
        currentNetworkTemplateController?.templates
      ) {
        return currentNetworkTemplateController.templates.filter(
      ...
      for (const key in masterTemplatesController?.templates) {
        if (
          Object.prototype.hasOwnProperty.call(
            masterTemplatesController?.templates,
            key
          )
        ) {
          const value = masterTemplatesController?.templates[key]
          if (value?.open && value?.templates) {
            const currentTemplates = value.templates.filter(
              (item: TemplateController) => {
                return (
                  item?.open &&
                  item?.chainId === chainId &&
